    ...
    try {
      AuthService().login(email: email, password: password).then((response) {
        ...
        if (response.statusCode == 200) {
          ...
          var username = (responseUser["username"] != null && responseUser["username"] != '') ? responseUser["username"] : responseUser["firstname"]+" "+responseUser["lastname"];

          _saveData(responseUser["id"], responseUser["email"], username, responseUser["token"]);
  ...
  registerUser(var firstname, var lastname, var email, var password) {
    ...
    try {
      AuthService()
          ...
          .then((response) {
            print(response.body);
            var message = response.body != null ? response.body['message'] ?? 'Veuillez réesayer plus tard.' : 'Veuillez réesayer plus tard.';
